...
export async function GET() {
  return Response.json({
    message: "Clartity explain API is running. Use POST to generate artwork explanation.",
  });
}
...
OCR 특성상 오타, 줄바꿈 오류, 언어 혼합, 인식 누락이 있을 수 있다.
...
4. 미술 기초지식이 없는 20-30대도 이해할 수 있게 설명해라.
5. 사용자가 질문했다면 answer 필드에 질문에 대한 직접 답변을 써라.
6. 반드시 JSON만 출력해라. 마크다운, 코드블록, 설명문은 출력하지 마라.
...
출력 JSON 형식:
{
  ...
  "explanation": "입문자용 작품 설명. 무엇을 그렸는지, 왜 흥미로운지 포함",
  "artistIntention": "작가의 의도 또는 확인 필요",
...
`;
    ...
    if (!parsed) {
      return Response.json({
        ...
        explanation: raw || "AI 해설을 생성했지만 JSON 변환에 실패했습니다.",
    ...
    return Response.json({
      ...
      explanation: parsed.explanation || "작품 설명을 생성하지 못했습니다.",
